...
"use client";
import { useState } from "react";
import { db, auth } from "../firebase"; // Import auth to get user ID
...
export default function AddReceiptPage() {
  ...
  // 🔹 Handle form submission to add receipt
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push("/add"); // Redirect to edit page
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* 🔹 Submit Button */}
        <button type="submit" className="bg-blue-500 text-white p-2 rounded mt-20">
          Add Receipt
        </button>
      </form>
    {/* Back to Main Page */}
      <Link href="/">
        <button className="bg-gray-500 text-white p-2 rounded mt-20">
          Back to Home
        </button>
      </Link>
    </div>
