...
export default function SummaryPage() {
  ...
  function isLast30Days(dateOrString: any) {
    let d: Date;
    if (typeof dateOrString === "string") {
      // Assume MM/DD/YYYY
      const [mm, dd, yyyy] = dateOrString.split("/");
      d = new Date(Number(yyyy), Number(mm) - 1, Number(dd));
    } else if (dateOrString && dateOrString.toDate) {
      d = dateOrString.toDate();
    } else {
        ...
        <div className="flex-1">
          <div className="w-full max-w-md p-4 bg-gray-900 border-2 border-gray-300 rounded-lg shadow-lg mb-10">
            ...
            <ul>
              {Object.entries(categoryTotals)
                .sort((a, b) => b[1] - a[1]) // Sort descending by total spent
                .map(([category, total]) => (
          ...
          <div className="w-full max-w-md p-4 bg-gray-900 border-2 border-gray-300 rounded-lg shadow-lg mb-4">
            ...
            <ul>
              {expenses
                .filter((expense) =>
                  isLast30Days(expense.day ?? expense.timestamp)
                )
